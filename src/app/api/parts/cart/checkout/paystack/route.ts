...
import { createPartsPaystackPendingOrder } from "@/lib/parts-checkout";
...
export async function POST(req: Request) {
  ...
  if (needsChinaShippingChoice && !parsed.data.chinaShippingChoice) {
    return NextResponse.json(
      { error: "Select Air or Sea shipping for China in-stock items." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message === "IDENTITY_VERIFICATION_REQUIRED") {
      return NextResponse.json(
        {
          error: "Identity verification is required before this checkout can continue.",
  ...
  if (selectedProvider !== "PAYSTACK") {
    return NextResponse.json({ error: "Paystack is required for card/mobile checkout." }, { status: 409 });
  }
  ...
  try {
    const pending = await createPartsPaystackPendingOrder({
      userId: session.user.id,
    ...
    const payRef = `SDA-${nanoid(12).toUpperCase()}`;
    await prisma.payment.update({
      where: { id: pending.paymentId },
      data: { providerReference: payRef, idempotencyKey: payRef },
    });

    const callbackOrigin = await getPaystackCallbackOrigin();
    const callbackUrl = `${callbackOrigin}/checkout/return?reference=${encodeURIComponent(payRef)}`;
    const init = await paystackInitialize({
      email: session.user.email,
      amountMinorUnits: Math.round(pending.amountGhs * 100),
      reference: payRef,
      currency: "GHS",
      metadata: { orderId: pending.orderId, paymentId: pending.paymentId, kind: "parts_checkout" },
      callbackUrl,
      secretKey,
    });
