      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex min-w-0 flex-1 flex-wrap gap-2">
            {slots.map((entry, idx) =>
              entry ? (
                ...
                <div
                  key={`empty-${idx}`}
                  className="flex min-h-[52px] min-w-[8rem] flex-1 items-center justify-center rounded-xl border border-dashed border-border px-3 text-xs text-muted-foreground dark:border-white/15"
                >
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          ...
          {compareHref ? (
            ...
            <span className="inline-flex h-9 items-center rounded-lg bg-muted/60 px-3 text-xs text-muted-foreground dark:bg-white/[0.04]">
              Pick {max - entries.length} more
            </span>
          )}
          <Link href="/inventory" className="inline-flex h-9 items-center justify-center px-2 text-xs font-medium text-[var(--brand)] hover:underline">
            Browse cars
          </Link>
        </div>
      </div>
